...
class CrawlerRequest(BaseModel):
    ...
    crawler_engine: str = Field(
        default="apify", description="Crawler engine ('apify' or 'crawl4ai')"
    )
    crawler_type: Optional[CrawlerType] = Field(
        default="cheerio", description="Crawler type (only for Apify)"
    )
            ...
            try:
                ...
                if request.crawler_engine.lower() == "crawl4ai":
                    ...
                    documents = await crawler.get(
                        urls_to_crawl, crawler_type=crawler_type
                    )
