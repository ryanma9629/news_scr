...
class CrawlerRequest(BaseModel):
    ...
    contents_load: bool = Field(default=True, description="Load contents from storage if possible")
    contents_save_days: int = Field(default=90, description="Only update contents older than this many days")
    contents_load_days: int = Field(default=90, description="Only load contents no older than this many days")
    session_id: Optional[str] = Field(None, description="Session ID for data persistence")
...
class TaggingRequest(BaseModel):
    ...
    tags_load: bool = Field(default=True, description="Load tags from storage if possible")
    tags_save_days: int = Field(default=90, description="Only update tags older than this many days")
    tags_load_days: int = Field(default=90, description="Only load tags no older than this many days")
    session_id: Optional[str] = Field(None, description="Session ID for data persistence")
...
def get_cors_origins():
    ...
    cors_origins = [
        "https://localhost:8280", "https://127.0.0.1:8280", "http://localhost:8280", "http://127.0.0.1:8280",
        "https://localhost", "https://127.0.0.1", "http://localhost", "http://127.0.0.1",
        "http://sasserver.demo.sas.com", "https://sasserver.demo.sas.com", "http://sasserver", "https://sasserver",
    ]
...
async def search_news(http_request: Request, request: SearchRequest):
    ...
    try:
        ...
        try:
            ...
            return SearchResponse(success=False, results=[], total_results=0, message=str(e.detail), session_id=session_id)

        keywords = get_search_keywords(request.company_name, request.search_suffix, request.lang)
        ...
        update_session_data(session_id, "search_results", [{"url": result["url"], "title": result["title"]} for result in results])
        ...
        return SearchResponse(success=False, results=[], total_results=0, message=f"Search error: {str(e)}", session_id=request.session_id)
...
async def crawl_news_content(request: CrawlerRequest):
    ...
    try:
        ...
        is_valid, validation_error = validate_session_and_urls(session_id, request.urls, "crawling")
        if not is_valid:
            return CrawlerResponse(success=False, results=[], total_results=0, message=validation_error or "Validation failed")
        ...
        if request.contents_load:
            contents_from_db = handle_storage_operation(
                mongo_store.load_contents, "loaded contents from MongoDB", request.urls, days=request.contents_load_days
            ) or []
            
            stored_urls = {content["url"] for content in contents_from_db}
            for content in contents_from_db:
                results.append(CrawlerResultResponse(url=content["url"], success=True, content=content["text"], error=None))
        ...
        if urls_to_crawl:
            ...
            try:
                ...
                for url in urls_to_crawl:
                    results.append(CrawlerResultResponse(url=url, success=False, content=None, error=f"Crawling failed: {crawler_error}"))

        # Save to storage and session
        if request.contents_save and crawled_contents:
            handle_storage_operation(mongo_store.save_contents, "saved contents to MongoDB", crawled_contents, days=request.contents_save_days)
        ...
        failed_results = [CrawlerResultResponse(url=url, success=False, error=f"System error: {e}", content=None) for url in request.urls]
        return CrawlerResponse(success=False, results=failed_results, total_results=len(failed_results), message=f"System error: {e}")
...
async def tag_news_content(request: TaggingRequest):
    ...
    try:
        ...
        try:
            ...
            return TaggingResponse(success=False, results=[], total_results=0, message=str(e.detail))

        is_valid, validation_error = validate_session_and_urls(session_id, request.urls, "tagging")
        if not is_valid:
            return TaggingResponse(success=False, results=[], total_results=0, message=validation_error or "Validation failed")
        ...
        if request.tags_load:
            tags_from_db = handle_storage_operation(
                mongo_store.load_tags, "loaded tags from MongoDB", request.urls,
                method=request.tagging_method, llm_name=request.llm_model, days=request.tags_load_days
            ) or []

            stored_urls = {tag["url"] for tag in tags_from_db}
            for tag in tags_from_db:
                results.append(TaggingResultResponse(
                    url=tag["url"], success=True, crime_type=tag.get("crime_type"),
        ...
        # Tag remaining URLs
        if urls_to_tag:
            contents_to_tag, urls_without_content = load_from_storage_with_fallback(mongo_store, urls_to_tag, session_id or "")

            for url in urls_without_content:
                results.append(TaggingResultResponse(
                    url=url, success=False, crime_type=None, probability=None,
            ...
            if contents_to_tag:
                ...
                for content in contents_to_tag:
                    ...
                    try:
                        ...
                        tag_result = await fc_tagging.tagging_rag(docs) if request.tagging_method == "rag" else await fc_tagging.tagging_combine(docs)

                        results.append(TaggingResultResponse(
                            url=url, success=True, crime_type=tag_result.get("crime_type"),
                            probability=tag_result.get("probability"), error=None
                        ))
                        tagged_results.append({
                            "url": url, "crime_type": tag_result.get("crime_type"),
                            "probability": tag_result.get("probability"), "method": request.tagging_method
                        })
                    except Exception as tag_error:
                        results.append(TaggingResultResponse(
                            url=url, success=False, crime_type=None, probability=None, error=f"Tagging failed: {tag_error}"
                        ))

                if tagged_results and request.tags_save:
                    handle_storage_operation(
                        mongo_store.save_tags, "saved tags to MongoDB", tagged_results,
                        method=request.tagging_method, llm_name=request.llm_model, days=request.tags_save_days
                    )
        ...
        failed_results = [TaggingResultResponse(url=url, success=False, error=f"System error: {e}", crime_type=None, probability=None) for url in request.urls]
        return TaggingResponse(success=False, results=failed_results, total_results=len(failed_results), message=f"System error: {e}")
...
async def summarize_news_content(request: SummaryRequest):
    ...
    try:
        ...
        if request.summary_level not in valid_levels:
            return SummaryResponse(success=False, message=f"Invalid summary level '{request.summary_level}'. Valid options: {', '.join(valid_levels)}", summary=None)

        if not session_id:
            return SummaryResponse(success=False, message="Session is required. Please search for news and get content first, then try summarization again.", summary=None)

        if not request.urls:
            return SummaryResponse(success=False, message="No URLs provided for summarization", summary=None)

        contents, missing_urls, error_msg = get_contents_from_session_with_validation(session_id, request.urls, "summarization")
        if error_msg:
            return SummaryResponse(success=False, message=error_msg, summary=None)

        try:
            llm, emb = init_llm_and_embeddings(deployment, request.llm_model)
            docs = [Document(page_content=content["text"], metadata={"url": content["url"]}) for content in contents if content.get("text")]
            ...
            summary = await summarizer.summarize(docs=docs, lang=request.lang, summary_level=request.summary_level, num_cluster=num_clusters)
            return SummaryResponse(success=True, message=f"Summary generated successfully, processed {len(contents)} articles", summary=summary)

        except Exception as summarization_error:
            logger.error(f"Summarization execution failed: {summarization_error}")
            return SummaryResponse(success=False, message=f"Summary generation failed: {summarization_error}", summary=None)

    except Exception as e:
        logger.error(f"Summary error: {e}")
        return SummaryResponse(success=False, message=f"System error: {e}", summary=None)
...
async def qa_endpoint(request: QARequest):
    ...
    try:
        ...
        try:
            ...
            return QAResponse(success=False, message=str(e.detail), question=request.question, answer=None, urls=[])

        is_valid, validation_error = validate_session_and_urls(session_id, request.urls, "Q&A")
        if not is_valid:
            return QAResponse(success=False, message=validation_error or "Validation failed", question=request.question, answer=None, urls=[])

        contents, missing_urls, error_msg = get_contents_from_session_with_validation(session_id or "", request.urls, "Q&A")
        if error_msg:
            return QAResponse(success=False, message=error_msg, question=request.question, answer=None, urls=[])

        llm, embeddings = init_llm_and_embeddings(deployment, request.llm_model)
        documents = [Document(page_content=content["text"], metadata={"url": content["url"]}) for content in contents if content.get("text")]
        
        if not documents:
            return QAResponse(success=False, message="No valid document content found for answering the question", question=request.question, answer=None, urls=[])

        text_splitter = RecursiveCharacterTextSplitter(chunk_size=DEFAULT_CHUNK_SIZE, chunk_overlap=DEFAULT_CHUNK_OVERLAP)
        split_docs = text_splitter.split_documents(documents)
        qa_system = QAWithContext(llm=llm, emb=embeddings)
        result = await qa_system.query(query=request.question, lang=request.lang, docs=split_docs)

        return QAResponse(
            success=True, message="Q&A processing successful", question=result.get("question", request.question),
            answer=result.get("answer"), urls=result.get("urls", [])
        )

    except Exception as e:
        logger.error(f"Error in QA processing: {str(e)}")
        return QAResponse(success=False, message=f"Q&A processing failed: {str(e)}", question=request.question, answer=None, urls=[])
